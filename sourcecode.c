...
int hasRecentlyPlayedAway(char *team, int currentMatchIndex)
{
    if (currentMatchIndex > 0)
    {
        if (strcmp(matchDetails[currentMatchIndex - 1].team2, team) == 0)
        {
            return 1;
        }
    }
...
void matchScheduling()
{
        ...
        for (int i = 0; i < MAX_TEAMS / 2 ; i++)
        {
            ...
            while (!foundSlot && searchAttempts < 365)
            {
                ...
                if (!isDateUnavailable(teams[finalHomeIdx].homeground, temp_day, temp_month) &&
                    !dayIsFull &&
                    !hasRecentlyPlayedAway(teams[finalAwayIdx].name, matchIndex))
                {
                    foundSlot = 1;
            ...
            scheduleMatch(finalHomeIdx, finalAwayIdx, current_day, current_month, matchCountToday);
    ...
    printf("Finished Phase 1 Scheduling Attempt. Matches scheduled: %d\n", scheduledMatchesCount);
    ...
    while (scheduledMatchesCount < totalMatchesTarget && attempts_phase2 < max_attempts_phase2)
    {
        ...
        { 
            ...
            if (lastmatch[finalHomeIdx].lastMatchDay != -1 && lastmatch[finalHomeIdx].lastMatchWasAway && daysBetween(temp_day, temp_month, temp_year, lastmatch[finalHomeIdx].lastMatchDay, lastmatch[finalHomeIdx].lastMatchMonth, temp_year) < 2)
            {
                gapOk = 0;
            }
            if (gapOk && lastmatch[finalAwayIdx].lastMatchDay != -1 && lastmatch[finalAwayIdx].lastMatchWasAway && daysBetween(temp_day, temp_month, temp_year, lastmatch[finalAwayIdx].lastMatchDay, lastmatch[finalAwayIdx].lastMatchMonth, temp_year) < 2)
            {
                gapOk = 0;
            }

            if (!isDateUnavailable(teams[finalHomeIdx].homeground, temp_day, temp_month) && !dayIsFull && gapOk)
            {
                foundSlot = 1;
                current_day = temp_day;
                current_month = temp_month;
    ...
    printf("\nFinished Scheduling. Total matches scheduled: %d\n", scheduledMatchesCount);
}
