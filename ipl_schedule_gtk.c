...
// Other helpers (isLeapYear, etc. - can be added back if needed for data processing)

// --- CSV Reading Functions --- (Assumed Correct from previous versions)
int read_schedule(const char *filename)
{
    FILE *file = fopen(filename, "r");
    if (!file) { perror("Error opening schedule file"); return 0; }
    char line[MAX_LINE_LEN], header_buffer[MAX_LINE_LEN];
    schedule_count = 0;
    if (fgets(header_buffer, sizeof(header_buffer), file) == NULL) { fclose(file); return 0; } // Skip header
    char *token, temp_line[MAX_LINE_LEN]; int line_num = 1;
    while (fgets(line, sizeof(line), file) && schedule_count < MAX_SCHEDULE_ENTRIES) {
        line_num++;
        if (strlen(line) < 5 || line[0] == '\n' || line[0] == ',') continue;
        line[strcspn(line, "\r\n")] = 0;
        strncpy(temp_line, line, sizeof(temp_line) - 1); temp_line[sizeof(temp_line) - 1] = '\0';
        char *fields[6] = {NULL}; int field_count = 0;
        token = strtok(temp_line, ",");
        while (token != NULL && field_count < 6) { fields[field_count++] = trimwhitespace(token); token = strtok(NULL, ","); }
        if (field_count >= 5) {
            strncpy(schedule[schedule_count].team1, fields[0], sizeof(schedule[schedule_count].team1) - 1);
            strncpy(schedule[schedule_count].team2, fields[1], sizeof(schedule[schedule_count].team2) - 1);
            strncpy(schedule[schedule_count].venue, fields[2], sizeof(schedule[schedule_count].venue) - 1);
            schedule[schedule_count].day = atoi(fields[3]); schedule[schedule_count].month = atoi(fields[4]);
            schedule[schedule_count].team1[sizeof(schedule[schedule_count].team1) - 1] = '\0';
            schedule[schedule_count].team2[sizeof(schedule[schedule_count].team2) - 1] = '\0';
            schedule[schedule_count].venue[sizeof(schedule[schedule_count].venue) - 1] = '\0';
            if (field_count == 6 && fields[5] != NULL && strlen(fields[5]) > 1) {
                 strncpy(schedule[schedule_count].time, fields[5], sizeof(schedule[schedule_count].time) - 1);
                 schedule[schedule_count].time[sizeof(schedule[schedule_count].time) - 1] = '\0';
            } else { strcpy(schedule[schedule_count].time, "TBC"); }
            if (schedule[schedule_count].day == 0 || schedule[schedule_count].month == 0) { fprintf(stderr, "Warning: Invalid date format on line %d of %s: %s\n", line_num, filename, line); continue; }
            sprintf(schedule[schedule_count].date, "%02d/%02d", schedule[schedule_count].day, schedule[schedule_count].month);
            schedule[schedule_count].no = schedule_count + 1; schedule_count++;
        } else { fprintf(stderr, "Warning: Malformed line %d in %s (fields: %d): %s\n", line_num, filename, field_count, line); }
    } fclose(file); printf("Read %d schedule entries from %s\n", schedule_count, filename); return schedule_count > 0;
}

int read_match_results(const char *filename)
{
    FILE *file = fopen(filename, "r");
    if (!file) { perror("Error opening match results file"); return 0; }
    char line[MAX_LINE_LEN], header_buffer[MAX_LINE_LEN];
    results_count = 0;
    if (fgets(header_buffer, sizeof(header_buffer), file) == NULL) { fclose(file); return 0; } // Skip header
    char *token, temp_line[MAX_LINE_LEN]; int line_num = 1;
    while (fgets(line, sizeof(line), file) && results_count < MAX_RESULTS_ENTRIES) {
        line_num++;
        if (strlen(line) < 10 || line[0] == '\n' || line[0] == ',') continue;
        line[strcspn(line, "\r\n")] = 0;
        strncpy(temp_line, line, sizeof(temp_line) - 1); temp_line[sizeof(temp_line) - 1] = '\0';
        char *fields[11] = {NULL}; int field_count = 0;
        token = strtok(temp_line, ",");
        while (token != NULL && field_count < 11) { fields[field_count++] = trimwhitespace(token); token = strtok(NULL, ","); }
        if (field_count == 11) {
            results[results_count].no = atoi(fields[0]);
            strncpy(results[results_count].date, fields[1], sizeof(results[results_count].date) - 1);
            strncpy(results[results_count].time, fields[2], sizeof(results[results_count].time) - 1);
            strncpy(results[results_count].team1, fields[3], sizeof(results[results_count].team1) - 1);
            strncpy(results[results_count].team2, fields[4], sizeof(results[results_count].team2) - 1);
            strncpy(results[results_count].venue, fields[5], sizeof(results[results_count].venue) - 1);
            results[results_count].runs1 = atoi(fields[6]); results[results_count].overs1 = atof(fields[7]);
            results[results_count].runs2 = atoi(fields[8]); results[results_count].overs2 = atof(fields[9]);
            strncpy(results[results_count].winner, fields[10], sizeof(results[results_count].winner) - 1);
            results[results_count].date[sizeof(results[results_count].date) - 1] = '\0';
            results[results_count].time[sizeof(results[results_count].time) - 1] = '\0';
            results[results_count].team1[sizeof(results[results_count].team1) - 1] = '\0';
            results[results_count].team2[sizeof(results[results_count].team2) - 1] = '\0';
            results[results_count].venue[sizeof(results[results_count].venue) - 1] = '\0';
            results[results_count].winner[sizeof(results[results_count].winner) - 1] = '\0';
             if (results[results_count].no == 0 && strcmp(fields[0],"0")!=0) { fprintf(stderr, "Warning: Invalid match number on line %d of %s: %s\n", line_num, filename, line); continue; }
            results_count++;
        } else { fprintf(stderr, "Warning: Malformed line %d in %s (fields: %d): %s\n", line_num, filename, field_count, line); }
    } fclose(file); printf("Read %d match result entries from %s\n", results_count, filename); return results_count > 0;
}

int read_points_table(const char *filename)
{
     FILE *file = fopen(filename, "r");
    if (!file) { perror("Error opening points table file"); return 0; }
    char line[MAX_LINE_LEN], header_buffer[MAX_LINE_LEN];
    points_count = 0;
    if (fgets(header_buffer, sizeof(header_buffer), file) == NULL) { fclose(file); return 0; } // Skip header
    char *token, temp_line[MAX_LINE_LEN]; int line_num = 1;
    while (fgets(line, sizeof(line), file) && points_count < MAX_TEAMS) {
        line_num++;
        if (strlen(line) < 5 || line[0] == '\n' || line[0] == ',') continue;
        line[strcspn(line, "\r\n")] = 0;
        strncpy(temp_line, line, sizeof(temp_line) - 1); temp_line[sizeof(temp_line) - 1] = '\0';
        char *fields[7] = {NULL}; int field_count = 0;
        token = strtok(temp_line, ",");
        while (token != NULL && field_count < 7) { fields[field_count++] = trimwhitespace(token); token = strtok(NULL, ","); }
        if (field_count == 7) {
            points[points_count].pos = atoi(fields[0]);
            strncpy(points[points_count].team, fields[1], sizeof(points[points_count].team) - 1);
            points[points_count].mp = atoi(fields[2]); points[points_count].won = atoi(fields[3]);
            points[points_count].lost = atoi(fields[4]); points[points_count].pts = atoi(fields[5]);
            points[points_count].nrr = atof(fields[6]);
            points[points_count].team[sizeof(points[points_count].team) - 1] = '\0';
             if (points[points_count].pos == 0 && strcmp(fields[0],"0")!=0) { fprintf(stderr, "Warning: Invalid position on line %d of %s: %s\n", line_num, filename, line); continue; }
            points_count++;
        } else { fprintf(stderr, "Warning: Malformed line %d in %s (fields: %d): %s\n", line_num, filename, field_count, line); }
    } fclose(file); printf("Read %d points table entries from %s\n", points_count, filename); return points_count > 0;
}
...
// *** MODIFIED Helper: Added foreground binding ***
GtkTreeViewColumn *create_text_column(const char *title, int col_id, gfloat xalign, int bg_col_id, int fg_col_id) // Added fg_col_id
{
    ...
    // *** Bind cell background color attribute ***
    gtk_tree_view_column_add_attribute(column, renderer, "cell-background", bg_col_id);
    // *** Bind cell foreground (text) color attribute ***
    ...
    // Adjust min widths (same as before)
...
GtkWidget *create_schedule_page()
{
    ...
    // *** MODIFIED ListStore: Added G_TYPE_STRING for foreground color ***
...
GtkWidget *create_results_page()
{
    ...
    GtkTreeViewColumn *col;
    // *** MODIFIED calls to create_text_column ***
...
GtkWidget *create_points_page()
{
    ...
    for (int i = 0; i < points_count; i++) {
        ...
        gtk_list_store_set(ls, &iter,
                           ...
                           POINTS_COL_FG_COLOR, text_fg_color_str, // *** Set foreground color string ***
                           -1);
    }
    GtkWidget *tv = gtk_tree_view_new_with_model(GTK_TREE_MODEL(ls));
    gtk_widget_set_name(tv, "ipl-treeview");
...
static void activate(GtkApplication *app, gpointer user_data)
{
    ...
    if (!pixbuf) {
        ...
        gtk_widget_set_hexpand(logo_image, FALSE); 
        gtk_widget_set_vexpand(logo_image, FALSE); 
